...
v30.0 | Architecture(Major): 러비스킬 검토 반영. §OBSIDIAN DC언급 삭제(→MCP_SPEED 일원화). §INFO_BRANCH decision/reference 예시 추가. §CONFIDENCE SKIP 'execution report'→'tool-result report'. §NUM_VERIFY '6-axis full'→'cross-verified full'. §MCP_SPEED 약어정의(FS·DC) 삽입.
  PREV_CHANGELOG: Agent-Ops/_archive/UP_user-preferences_v29.7.md
...
CONFIDENCE ::= "확신도: N(basis)" @ decision-response end | ENUM {90:verified 70:inferred 50:assumed 30:speculative} only
  ...
  SKIP: casual chat, tool-result report | verification incomplete → ≤30
...
OBSIDIAN ::= MCP(obsidian) read-only HARD | write·edit·create·delete via MCP = BLOCK(무조건 거부, 우회 불가) | 볼트 쓰기 → FS or Cowork file tools(Write·Edit) only | 위반 = FAIL

MCP_SPEED ::= 파일접근우선순위: FS(mcp-filesystem) > Cowork빌트인(Read·Write·Edit) > DC(Desktop Commander) | DC=터미널·프로세스·xlsx/docx/pdf편집전용
